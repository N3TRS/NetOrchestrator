...
describe("OrchestratorService", () => {
  ...
  describe("clearJob", () => {
    ...
    it("rethrows non-404 errors", async () => {
      ...
      await expect(service.clearJob({ jobName: "my-job" })).rejects.toThrow(
        "K8s server error",
      );
    });
  });
  ...
  describe("runningProject", () => {
    ...
    it("rethrows K8s API errors", async () => {
      ...
      await expect(service.runningProject(dto)).rejects.toThrow(
        "K8s unavailable",
      );
    });
  });
    ...
    it("emits logs:error when log read fails on Succeeded pod", async () => {
      ...
      expect(mockClient.emit).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          message: expect.stringContaining("log stream broken"),
        }),
      );
    });
