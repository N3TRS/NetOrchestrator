...
describe('CreateRunOrchestratorDto', () => {
  ...
  it('accepts single-segment version', async () => {
    const dto = plainToInstance(CreateRunOrchestratorDto, { ...valid, JAVA_VERSION: '17' });
    const errors = await validate(dto);
    expect(errors).toHaveLength(0);
  });
  ...
  it('rejects empty REPO_URL', async () => {
    const dto = plainToInstance(CreateRunOrchestratorDto, { ...valid, REPO_URL: '' });
    const errors = await validate(dto);
    expect(errors.some((e) => e.property === 'REPO_URL')).toBe(true);
  });

  it('rejects alphabetic JAVA_VERSION', async () => {
    const dto = plainToInstance(CreateRunOrchestratorDto, { ...valid, JAVA_VERSION: 'seventeen' });
    const errors = await validate(dto);
  ...
  it('rejects empty JAVA_VERSION', async () => {
    const dto = plainToInstance(CreateRunOrchestratorDto, { ...valid, JAVA_VERSION: '' });
    const errors = await validate(dto);
    expect(errors.some((e) => e.property === 'JAVA_VERSION')).toBe(true);
  });
});
...
describe('ClearJobDto', () => {
  it('accepts valid kubernetes job name', async () => {
    const dto = plainToInstance(ClearJobDto, { jobName: 'maven-generator-123' });
    const errors = await validate(dto);
    expect(errors).toHaveLength(0);
  });
