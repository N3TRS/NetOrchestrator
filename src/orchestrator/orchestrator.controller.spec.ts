...
describe('OrchestratorController', () => {
  ...
  describe('runProject', () => {
    it('delegates to orchestratorService.runningProject and returns result', () => {
      const dto = { REPO_URL: 'https://github.com/user/repo', JAVA_VERSION: '17' };
      const expected = { message: 'Job created', jobName: 'maven-generator-123' };
  ...
  describe('getJavaVersion', () => {
    ...
    it('returns javaVersion on success', async () => {
      mockOrchestratorService.javaVersion.mockResolvedValue({ javaVersion: '17' });
    ...
    it('throws 504 HttpException on timeout error', async () => {
      ...
      await expect(controller.getJavaVersion(dto as any)).rejects.toThrow(HttpException);
  ...
  describe('clearJobs', () => {
    it('delegates to orchestratorService.clearJob and returns result', async () => {
      ...
      const expected = { message: 'Job deleted', jobName: 'maven-generator-123' };
